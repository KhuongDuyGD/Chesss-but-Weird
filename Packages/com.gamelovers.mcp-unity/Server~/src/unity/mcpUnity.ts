...
const MCP_UNITY_RUNTIME_PORT_PATH = path.resolve(process.cwd(), './Temp/McpUnityRuntimePort.json');
...
export class McpUnity {
  ...
  public async start(clientName?: string): Promise<void> {
    try {
      ...
      const config: UnityConnectionConfig = {
        ...
        portResolver: () => this.resolveUnityPort(),
  ...
  private async resolveUnityPort(): Promise<number> {
    try {
      const runtimeContent = await fs.readFile(MCP_UNITY_RUNTIME_PORT_PATH, 'utf-8');
      const runtimeConfig = JSON.parse(runtimeContent);
      const runtimePort = runtimeConfig.Port;
      if (runtimePort) {
        return parseInt(runtimePort, 10);
      }
    } catch (err) {
      this.logger.debug(`McpUnityRuntimePort.json not found or unreadable: ${err instanceof Error ? err.message : String(err)}`);
    }
