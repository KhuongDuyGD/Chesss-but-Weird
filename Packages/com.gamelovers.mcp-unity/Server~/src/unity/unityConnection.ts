...
export class UnityConnection extends EventEmitter {
  ...
  private async doConnect(): Promise<void> {
    ...
    if (this.config.portResolver) {
      try {
        const resolvedPort = await this.config.portResolver();
        if (resolvedPort !== this.config.port) {
          this.logger.info(`Unity port changed ${this.config.port} -> ${resolvedPort}`);
          this.config.port = resolvedPort;
        }
      } catch (err) {
        this.logger.warn(`Failed to resolve Unity port: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
