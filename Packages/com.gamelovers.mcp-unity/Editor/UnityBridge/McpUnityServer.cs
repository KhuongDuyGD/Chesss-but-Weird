...
namespace McpUnity.Unity
{
    ...
    {
        private const string RuntimePortPath = "Temp/McpUnityRuntimePort.json";
        ...
        public void StartServer()
        {
            ...
            var allowRemote = McpUnitySettings.Instance.AllowRemoteConnections;
            var host = allowRemote ? "0.0.0.0" : "localhost";
            var port = ResolveServerPort(allowRemote);

            try
            {
                if (McpUnitySettings.Instance.Port != port)
                {
                    McpLogger.LogInfo($"Port {McpUnitySettings.Instance.Port} is unavailable. Switching MCP Unity server to port {port}.");
                    McpUnitySettings.Instance.Port = port;
                    McpUnitySettings.Instance.SaveSettings();
                }

                WriteRuntimePortInfo(host, port);
            ...
            {
                McpLogger.LogError($"Failed to start WebSocket server: Port {port} is already in use. {ex.Message}");
            }
            catch (Exception ex)
            {
        ...
        private int ResolveServerPort(bool allowRemote)
        {
            var settings = McpUnitySettings.Instance;

            if (!settings.AutoSelectAvailablePort)
            {
                return settings.Port;
            }

            var preferredPort = settings.PreferredPort > 0 ? settings.PreferredPort : settings.Port;
            var searchRange = Math.Max(1, settings.PortSearchRange);

            if (CanBind(settings.Port, allowRemote))
            {
                return settings.Port;
            }
            ...
            if (CanBind(preferredPort, allowRemote))
            {
                return preferredPort;
            }

            for (var port = preferredPort; port < preferredPort + searchRange; port++)
            {
                if (CanBind(port, allowRemote))
                {
                    return port;
                }
            }
        ...
        private bool CanBind(int port, bool allowRemote)
        {
            TcpListener listener = null;

            try
            {
                var address = allowRemote ? System.Net.IPAddress.Any : System.Net.IPAddress.Loopback;
        ...
        private void WriteRuntimePortInfo(string host, int port)
        {
            try
            {
                Directory.CreateDirectory(Path.GetDirectoryName(RuntimePortPath) ?? "Temp");
                File.WriteAllText(RuntimePortPath, JsonUtility.ToJson(new RuntimePortInfo
                {
                    Host = host,
                    Port = port
                }, true));
            ...
            {
                McpLogger.LogError($"Failed to write MCP Unity runtime port file: {ex.Message}");
            }
        }
